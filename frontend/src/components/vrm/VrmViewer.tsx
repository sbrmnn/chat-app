...
export function VrmViewer({
  ...
  useEffect(() => {
    ...
    // Lighting — warm cream ambient + mustard key + orange rim
    scene.add(new THREE.AmbientLight(0xfff5e0, 0.85))
    const key = new THREE.DirectionalLight(0xfae0a0, 0.7)
    key.position.set(1, 2, 1)
    scene.add(key)
    const rim = new THREE.DirectionalLight(0xd4673a, 0.4)
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          ...
          {state === "loading" && (
            <div className="absolute bottom-8 flex flex-col items-center gap-2">
              <div className="h-1 w-32 overflow-hidden border border-brown-600 bg-cream-50">
                <div
              ...
              <div className="flex items-center gap-2 text-[10px] font-bold tracking-wider text-brown-700">
                <JP>読み込み中</JP>
          ...
          {state === "error" && (
            <div className="absolute bottom-8 flex flex-col items-center gap-1 text-center">
              <JP className="text-xs font-bold text-orange-400">
                読み込みに失敗しました
              </JP>
