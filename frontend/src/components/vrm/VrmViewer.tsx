...
export function VrmViewer({
  ...
  useEffect(() => {
    ...
    // Lighting — bright clean ambient + cool blue rim
    scene.add(new THREE.AmbientLight(0xffffff, 0.95))
    const key = new THREE.DirectionalLight(0xffffff, 0.7)
    key.position.set(1, 2, 1)
    scene.add(key)
    const rim = new THREE.DirectionalLight(0xa5c4ff, 0.4)
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          ...
          {state === "loading" && (
            <div className="absolute bottom-8 flex flex-col items-center gap-2">
              <div className="h-1 w-32 overflow-hidden rounded-full bg-white/40 backdrop-blur">
                <div
          ...
          {state === "error" && (
            <div className="absolute bottom-8 flex flex-col items-center gap-1 text-center">
              <JP className="text-xs font-semibold text-text-secondary">
                読み込みに失敗しました
              </JP>
